...
def lemmatize_verbs_spacy(words):
    """Lemmatize verbs in a list of tokenized words using spaCy"""
    # Join words into a string for spaCy processing
    text = ' '.join(words)
    doc = nlp(text)
    
    # Extract lemmatized verbs only
...
def lemmatize(words):
    lemmas = lemmatize_verbs_spacy(words)
    return lemmas
...
def normalize_and_lemmatize_spacy(input_text: str) -> str:
    # Step 1: remove special characters
    cleaned_text = remove_special_characters(input_text)
    
    # Step 2: process with spaCy
    doc = nlp(cleaned_text.lower())  # convert to lowercase
    
    # Step 3: normalize tokens (remove stopwords/punctuations optionally)
    lemmas = [token.lemma_ for token in doc if not token.is_stop and not token.is_punct]
    
    # Step 4: return as space-joined string
    return ' '.join(lemmas)
...
def recommend_products(user_name):
    ...
    output_df['lemmatized_text'] = output_df['reviews_text'].map(lambda text: normalize_and_lemmatize_spacy(text))
